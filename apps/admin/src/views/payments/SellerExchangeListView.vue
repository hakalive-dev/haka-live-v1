...
const approveModal = ref<{ id: string; pointsAmount: number; user: any } | null>(null)
const rejectModal = ref<{ id: string; pointsAmount: number; user: any } | null>(null)
...
async function fetchData() {
  ...
  try {
    ...
    const result = await paymentsApi.listSellerExchanges(params)
...
async function confirmApprove() {
  ...
  try {
    await paymentsApi.approveSellerExchange(id)
    toast.success('Exchange approved — wallet points deducted and seller coins credited')
    await fetchData()
  } catch (e: any) {
...
async function confirmReject() {
  ...
  try {
    await paymentsApi.rejectSellerExchange(id, rejectNotes.value)
    toast.warning('Exchange rejected')
...
useAdminRealtime('seller_exchanges', fetchData)
...
<template>
  <div class="page">
    ...
    <p class="page-help">
      Sellers request converting wallet points (beans) into offline seller coin balance. Approval debits their wallet and credits their seller balance.
    </p>
    ...
    <div class="table-card">
      <div v-if="loading" class="loading">Loading exchange requests...</div>
      <div v-else-if="requests.length === 0" class="empty">No exchange requests found.</div>
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-for="req in requests" :key="req.id">
            ...
            <td class="bold">{{ Number(req.pointsAmount).toLocaleString() }}</td>
            ...
            <td class="actions-td">
              <RowActionMenu v-if="req.status === 'pending'">
                <RowActionMenuItem
                  ...
                  @click="approveModal = { id: req.id, pointsAmount: req.pointsAmount, user: req.seller }"
                >
                  Approve
                </RowActionMenuItem>
                <RowActionMenuItem
                  variant="danger"
                  :disabled="actionLoading === req.id"
                  @click="rejectModal = { id: req.id, pointsAmount: req.pointsAmount, user: req.seller }; rejectNotes = ''"
                >
                  Reject
                </RowActionMenuItem>
              </RowActionMenu>
    ...
    <div v-if="approveModal" class="modal-overlay" @click.self="approveModal = null">
      <div class="modal">
        <h3>Approve point exchange</h3>
        <p>
          Debit <strong>{{ Number(approveModal.pointsAmount).toLocaleString() }} wallet points</strong> from
          <strong>{{ approveModal.user?.displayName }}</strong> and credit the same amount to their seller offline
          balance?<br />
          The seller must still have enough points at approval time.
        </p>
    ...
    <div v-if="rejectModal" class="modal-overlay" @click.self="rejectModal = null">
      <div class="modal">
        <h3>Reject exchange</h3>
        <p>
          Reject {{ Number(rejectModal.pointsAmount).toLocaleString() }} points from
          <strong>{{ rejectModal.user?.displayName }}</strong>. No wallet or seller balance changes.
        </p>
...
.page-help { font-size: 13px; color: #666; margin: 0 0 16px; max-width: 720px; line-height: 1.5; }
.stat-pill { background: #f0f0f0; border-radius: 12px; padding: 4px 12px; font-size: 13px; color: #555; }
.toolbar { display: flex; gap: 12px; margin-bottom: 16px; flex-wrap: wrap; }
.search-input { flex: 1; min-width: 160px; height: 38px; padding: 0 12px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; outline: none; }
