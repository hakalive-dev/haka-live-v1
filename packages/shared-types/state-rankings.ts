/** State / province subdivision for country-scoped host rankings. */
export type StateSubdivision = {
  code: string;
  name: string;
};

export type StateRankingCountryConfig = {
  countryCode: string;
  countryName: string;
...
export const STATE_RANKING_COUNTRIES: StateRankingCountryConfig[] = [
  {
    countryCode: 'IN',
    countryName: 'India',
    states: INDIA_STATES,
...
/** Normalize country to ISO-2 when possible. */
export function normalizeCountryCode(country: string): string {
  ...
  const byName = STATE_RANKING_COUNTRIES.find(
    (c) => c.countryName.toLowerCase() === raw.toLowerCase(),
  );
  return byName?.countryCode ?? raw.toUpperCase();
}

export function getStateRankingCountry(countryCode: string): StateRankingCountryConfig | null {
  const code = normalizeCountryCode(countryCode);
  return STATE_RANKING_COUNTRIES.find((c) => c.countryCode === code) ?? null;
}

export function isStateRankingEnabled(countryCode: string): boolean {
  return getStateRankingCountry(countryCode) != null;
}

export function getStatesForCountry(countryCode: string): StateSubdivision[] {
  return getStateRankingCountry(countryCode)?.states ?? [];
}

export function getStateName(countryCode: string, stateCode: string): string | null {
  const states = getStatesForCountry(countryCode);
  const code = stateCode.trim().toUpperCase();
  return states.find((s) => s.code.toUpperCase() === code)?.name ?? null;
}
...
export function poolForStateRank(
  stateRank: number,
  tiers: StateRankTier[] = DEFAULT_STATE_RANK_REWARD_TIERS,
): number {
...
export function hostRewardAmount(poolTotal: number, hostRank: 1 | 2 | 3 | 4): number {
  return Math.floor(poolTotal * DEFAULT_HOST_REWARD_SPLITS[hostRank - 1]!);
}

/** Sum of tier pools for the first N ranked states (for daily prize pool display). */
export function totalDailyPrizePoolForStateCount(
